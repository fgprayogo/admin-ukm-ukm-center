     ...
     :
      <Fragment>
        <div className="disable-text-selection">
          ...
          <Row>
            {this.props.aktif.map(data => {
              {/* disini cek if image list    */}
...
const mapStateToProps = ({ authUser, pendaftaranReducer }) => {
  const { user, loading, profile} = authUser;
  const {aktif } = pendaftaranReducer;
  return { user, loading, aktif, profile};
};
